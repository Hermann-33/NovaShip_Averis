...
export function SevenFieldCard({ cmp, reviewReason, onEvidence }: { cmp: Comparison | null; reviewReason?: string | null; onEvidence?: (f: ComparisonField) => void }) {
  ...
  return (
    <div className={`overflow-hidden rounded-2xl border-2 bg-white shadow-card ${tone}`}>
      <div className="flex flex-wrap items-center gap-3 border-b border-ink-100 bg-white/80 px-5 py-4">
        <div>
      ...
      <div className="border-t border-ink-100 bg-ink-50/70 px-5 py-3 text-[11px] text-ink-500">Compared {new Date(cmp.compared_at).toLocaleString()} · normalization: case/whitespace/unit only (no legal-name rewriting) · a mismatch in one field never affects another.</div>
    </div>
  );
}
