...
export function SevenFieldCard({ cmp, reviewReason, onEvidence }: { cmp: Comparison | null; reviewReason?: string | null; onEvidence?: (f: ComparisonField) => void }) {
  ...
  if (!cmp) {
    return (
      <div className="rounded-xl border-2 border-dashed border-ink-200 bg-white p-5">
  ...
  return (
    <div className={`rounded-xl border-2 bg-white shadow-sm ${tone}`}>
      <div className="flex flex-wrap items-center gap-3 border-b border-ink-100 bg-white/70 px-4 py-3">
        <div>
      ...
      <div className="px-4 py-2 text-[11px] text-ink-500">Compared {new Date(cmp.compared_at).toLocaleString()} · normalization: case/whitespace/unit only (no legal-name rewriting) · a mismatch in one field never affects another.</div>
    </div>
  );
}
...
export function EvidencePanel({ cmp, docs, selected }: { cmp: Comparison | null; docs: Record<string, { name: string; text: string | null }>; selected?: ComparisonField | null }) {
  ...
  return (
    <div className="space-y-3">
      {fields.map((f) => (
        <div key={f.field} className={`rounded-lg border p-3 ${f.result === "MISMATCH" ? "border-mismatch/40 bg-mismatch-bg/30" : "border-ink-200"}`}>
