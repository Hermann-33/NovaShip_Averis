  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
      <div className="rounded-xl border border-ink-200 bg-white p-4">
        <div className="mb-2 flex items-center justify-between"><h3 className="text-sm font-semibold">LangGraph run</h3><div className="flex gap-1"><Button kind="primary" disabled={busy} onClick={run}>Run graph</Button><Button disabled={busy} onClick={load}>Refresh</Button></div></div>
        <p className="text-xs text-ink-500">security_precheck, security_agent, classify, detect_documents, extract, compare (deterministic), summarize_and_draft, human_review (interrupt), notify. Full console on <Link href="/agent" className="text-accent hover:underline">AI agent</Link>.</p>
        {!st || !st.trace?.length ? <div className="mt-3"><Empty text="No graph run for this case yet. The classic pipeline processed it; click Run graph to execute the LangGraph version with the security agent and the human-in-the-loop pause." /></div> : (
          <ol className="mt-3 space-y-1 text-xs">{st.trace.map((t: any, i: number) => <li key={i} className="rounded bg-ink-50 p-1.5"><b>{t.node}</b> <Badge className="bg-ink-100 text-ink-700">{t.actor_type}</Badge> <span className="font-mono text-[10px] text-ink-600">{JSON.stringify(t.output).slice(0, 200)}</span></li>)}</ol>
        )}
      </div>
